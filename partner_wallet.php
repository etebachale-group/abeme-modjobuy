<?php
// Partner wallet (refactor): function-based page using centralized helpers.
...
// Resolve partner and access rules
$partnerName = isset($_GET['partner']) ? trim($_GET['partner']) : '';
if ($partnerName === '') {
    http_response_code(400);
    echo 'Socio no especificado';
    exit;
}
// Redirect CAJA to its dedicated page
if (strcasecmp($partnerName, 'CAJA') === 0) {
    header('Location: caja.php');
    exit;
}
if (function_exists('requirePartnerAccess')) {
    requirePartnerAccess($partnerName);
}

// --- Local helpers ---
function ensure_schema(PDO $pdo): void {
    // Base table for partner benefits
    ...
    // Ensure wallet_balance exists for older DBs
    try {
...
if (($_SERVER['REQUEST_METHOD'] ?? 'GET') === 'POST') {
    ...
    try {
        if ($action === 'deposit') {
            ...
            // Deposit: reduce pendiente y aumentar monedero
            wallet_deposit($partnerName, $amount, 'admin_deposit', $notes, true, true);
            $flash = ['type'=>'success','msg'=>'Depósito realizado'];
            header('Location: partner_wallet.php?' . http_build_query(['partner'=>$partnerName, 'ok'=>1]));
            exit;
    } elseif ($action === 'withdraw') {
      if ($amount <= 0) { throw new Exception('Monto inválido'); }
      $res = wallet_withdraw($partnerName, $amount, 'partner_withdraw', $notes);
      ...
      // stay on page to show ticket link
        }
    } catch (Throwable $e) {
...
<head>
  ...
  <title>Monedero de <?php echo htmlspecialchars($partnerName); ?></title>
  ...
  <main class="main-content wallet-page">
    <section class="dashboard-section">
      <div class="wallet-hero">
        <div class="hero-top">
          <h1><i class="fas fa-wallet"></i> Monedero • <?php echo htmlspecialchars($partnerName); ?></h1>
        ...
        <form method="get" class="filters" style="display:grid; grid-template-columns: repeat(auto-fit,minmax(180px,1fr)); gap:12px; align-items:end; margin:10px 0 4px;">
          ...
          <div><label class="label">Método</label>
            <select name="method" class="form-input-dark">
              ...
              <option value="partner_withdraw" <?php echo $filters['method']==='partner_withdraw'?'selected':''; ?>>Retiro (Socio)</option>
          ...
          <div class="btn-group">
            ...
            <a class="btn btn-dark" href="partner_wallet.php?partner=<?php echo urlencode($partnerName); ?>"><i class="fas fa-undo"></i> Limpiar</a>
          </div>
        </form>
        ...
        <?php if ($pages > 1): ?>
          <div style="margin-top:10px; display:flex; gap:8px; flex-wrap:wrap;">
            <?php for($i=1;$i<=$pages;$i++): $q = $_GET; $q['page']=$i; ?>
              <a class="btn btn-dark<?php echo $i===$page?' active':''; ?>" href="<?php echo htmlspecialchars('partner_wallet.php?'.http_build_query($q)); ?>"><?php echo $i; ?></a>
            <?php endfor; ?>
          </div>
        <?php endif; ?>
      </div>
